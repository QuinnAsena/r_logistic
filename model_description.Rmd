...
Note that in this density-dependent model there are three points of stability: $N=0$ (extinction or extirpation) and $K$, but the third, zero growth at $N>0$, is dependent on the other inputs. Unlike equation 1, in equation 2, the equilibrium population size changes depending on the growth rate $\lambda$ (and so values of $\lambda$ that yield positive growth are dependent on starting population and carrying capacity). For example, if $\lambda = 2$ the population stabilises at $K/2$. This is not the case for the continuous logistic model, which is not covered here.
...
* $r_d < 2.000$: population reaches $K$ with damping oscillations
* $2.000 < r_d < 2.449$: two-point stable limit cycle
* $2.449 < r_d < 2.570$: limit cycles increase geometrically (periods of 2, 4, 8, 16...)
* $r_d > 2.570$: aperiodic (irregular) patterns known as chaos.

In equation 2 (Lambda discrete model tab)
* $\lambda < 3$: population reaches $K$ with damping oscillations
* $3.0 < \lambda < 3.4$: two-point stable limit cycle
* $3.4 < \lambda \lessapprox 3.6$: limit cycles increase geometrically (periods of 2, 4, 8, 16...)
...
The following assumptions underlie the general logistic growth model. The discrete models presented here relax the continuity assumption but share the remaining assumptions.

* __No time lags:__ the rate of increase responds instantaneously as population size changes. Growth rate remains constant through time without stochasticity.
* __No age, size, or sex structure:__ birth and death rates of the population do not change with the age or body size of individuals. Every individual added to the population is immediately reproductive and does not have life stages of greater or lower reproductive output. This assumption is more realistically relaxed for modelling bacteria or protozoa, for example.
* __No genetic structure:__ individuals in the population are assumed to have the same birth and death rates and no underlying genetic variation exists.
...
* __No immigration or emigration:__ immigration and emigration of individuals is assumed not to occur, or to be equal.

More complicated variations of these models address the limitations of the assumptions outlined above. In reality, violating some of these assumptions could be detrimental to the accuracy of population projections depending on the population of interest, although these simple models still illustrate the basic principles of logistic population growth.
